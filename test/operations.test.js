...
(async () => {
    ...
    await check('health and rates are served', async () => {
        ...
        assert.strictEqual(ready.json.status, 'ready', 'the openvibe-shared/ready shape');
        assert.strictEqual(ready.json.service, 'billing');
        assert.ok(ready.json.checks && ready.json.checks.db && ready.json.checks.network_jwks, 'named checks');
